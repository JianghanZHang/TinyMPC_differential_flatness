...
Building on this observation, we propose two extensions of TinyMPC:

\begin{enumerate}
  \item \textbf{Flat Template MPC} (Section~\ref{sec:flat_template}):
    Precompute DARE caches at a grid of $\balpha$ values offline.
    Online, look up the nearest cache and run $K$ ADMM iterations
    as in standard TinyMPC, but with $\balpha$-specific
  ...
  \item \textbf{Analytic Template} (Section~\ref{sec:analytic}):
    ...
    projection.
    The total cost is ${\sim}260$ multiply-accumulate operations.
\end{enumerate}

Both variants shift the coordinate frame to the flat equilibrium
$(x^*(\balpha), u^*(\balpha))$, so the ADMM solver always
operates in a neighborhood of zero error---regardless of the
trajectory aggressiveness.
Fig.~\ref{fig:hierarchy} illustrates the pipeline and the
relationship among the four controller variants.
...
\end{align}
via ADMM.
The key insight is that, for a sufficiently long horizon,
the Riccati recursion converges to the infinite-horizon solution
$(K_\infty, P_\infty)$.
TinyMPC caches
\begin{align}
  ...
  C_2      &= (A - B K_\infty)^\top, \notag
\end{align}
where $R_\rho = R + \rho I$ and $Q_\rho = P_{\mathrm{lqr}} + \rho I$
are the ADMM-augmented cost matrices.
...
The $12 \times 4$ matrix $B_c$ has nonzero blocks in
the velocity rows (thrust direction $\times k_t / m$) and
the angular velocity rows (torque allocation $J^{-1} M$),
where $M$ is the motor mixing matrix.
...
\begin{enumerate}
  ...
  \item Solve DLQR to obtain terminal cost $P_{\mathrm{lqr}}^{(j)}$.
...
\begin{enumerate}
  ...
  \item Run $K$ ADMM iterations using cache $j^*$
...
\begin{enumerate}
  ...
  \item Interpolate the gain (192 MACs):
...
The four variants thus form a chain, with each step trading
optimality for computational savings:
\begin{multline*}
  \text{Online Relin}
...
The four variants form a natural hierarchy
(see also the equivalence chain~\eqref{eq:chain}):
\begin{center}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
Variant & Online ops & Constraints & Scheduling \\
\midrule
Online Relin & $O(n^3) {+} K {\cdot} O(n^2)$ & Yes & $\R^n$ \\
TinyMPC & $K \cdot O(n^2)$ & Yes & fixed \\
Flat Template & $K \cdot O(n^2)$ & Yes & $\R^4$ grid \\
Analytic & $O(n \cdot m)$ & Clip only & $\R^4$ 1st order \\
\bottomrule
\end{tabular}
\end{center}
...
The Analytic Template trades constraint handling for extreme efficiency,
making it suitable for the innermost loop of a hierarchical controller
or for FPGA implementation.
...
and (iii)~showing the equivalence between the zero-iteration ADMM
...
without the flatness reduction.
...
We presented two extensions of TinyMPC that exploit differential
...
Preliminary simulation results demonstrate that both variants
